...
## Implementing root transition and disabling cross-fade

We want to add transition animations to the letters in the game above.

Whenever a letter is chosen, there are some changes of words happening in the DOM:
...
```js
  function chooseLetter(letter) {
    ...
    document.startViewTransition(() => { // wrap the DOM update in a view transition
      updateDOM(letter, indexes, result);
    })
  }
```
...
We don't need any cross-fade animation, so we disable it with the following CSS:
...
## If there is no ::view-transition-new, only ::view-transition-old exists

When a letter is chosen, if the letter is wrong, the letter psuodo element has only the old state, and no new state. Therefore we can add a pop-out animation for the wrong letter.
...
/*                      ^^^^^^^^^^^ only-child means there is no new state */
  animation: pop-out 0.25s ease-in;
}
```

The example above is used when an element leaves (because there is no new state). If it is when an element enters (without an old state), you can use

```css
::view-transition-new(*):only-child {
  /*                      ^^^^^^^^^^^ only-child means there is no old state */
...
Now there is no animation at all because all the page is treated as a root for the animation. We need to name all the letters in the DOM individually so that each letter has resize and position animations.
...
```js
  function initDOM() {
    ...
    letters.forEach((letter) => {
      ...
      letterWrapper.style.viewTransitionName = `letter-${letter}`; // give each letter a name for the transition
...
The movement effect of the letters is now happening on the keyboard part, but when selecting the correct letter, this letter still only appears with a pop-out effect, instead of moving to the guess position. Therefore, we need to assign the guessed letter the same viewTransitionName in populateGuess.
...
```js
  function populateGuess(letter, indexes) {
    ...
    indexes.forEach((index, _index) => {
      ...
      slotItem.innerText = letter.toUpperCase();
      slotItem.classList.add("letter", `letter-${letter}`);
      slotItem.style.viewTransitionName = `letter-${letter}`; // Add this line
      //                                  ^^^^^^^^^^^^^^^^^^ same with initDOM
      slots[index].appendChild(slotItem);
    });
  }
