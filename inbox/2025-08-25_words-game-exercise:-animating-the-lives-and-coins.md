...
# words game exercise: animating the lives and coins
    ...
    <div class="header">
      ...
      <div class="info">
        ...
        <button class="buy-button" disabled>
          1 <img src="./assets/heart-balloon-svgrepo-com.svg" alt="" /> for 25
          <img src="./assets/coins-currency-svgrepo-com.svg" alt="" />
        </button>
        <div class="coins">
          <img src="./assets/coins-currency-svgrepo-com.svg" alt="" />
          <div class="count">
            <span class="visually-hidden">Coins:</span>
...
## let's animate the lives when we buy a new life
...
## animate all the existing lives

We want to give each life a unique view transition name, we can find a function to fill lives, and then add a viewTransitionName to each heart.
  ...
  function populateLives() {
    ...
    Array(lives)
      ...
      .forEach((h, i) => {
        ...
        heart.src = "./assets/heart-balloon-svgrepo-com.svg";
        heart.alt = "";
        heart.style.viewTransitionName = `life-${i}`; // Add view transition name to each heart
        heartsEl.appendChild(heart);
      });
  }
```

## animate the live just adding

Because the new live will not have ::view-transition-old, we can use `only-child` to select it and set the animation.
...
## Only animate when the points change

Even if the value of the coins above does not change, the animation will still be triggered. We can change it in JS to dynamically add the view-transition-name based on whether the value has changed to prevent unnecessary animations.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function chooseLetter(letter) {
    // ...

    if (indexes.length > 0) {
      // add view transition name to coins text element to animate it if points change
...
//  ^^^^^^^^^^^^^^^^^^
    ...
    // wait for the transition to finish before removing the view transition name
  ...
  async function buyLife() {
    if (points < LIFE_PRICE) return;
    coinsAudio.play();
    coinsTextEl.style.viewTransitionName = "coins"; // add view transition name to coins text element to animate it if points change
    points -= LIFE_PRICE;
    lives += 1;
